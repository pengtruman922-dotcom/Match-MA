...
**只有相关服务会重建**：五个服务的 `railway*.toml` 都配了 `build.watchPatterns`。
后端四个服务看 `backend/ scripts/ alembic/ database/ pyproject.toml railway*.toml`，
前端只看 `frontend/**`。所以纯文档提交、只改 `tests/` 或只改 `deploy/` 的提交**一个服务都不会重建**
（2026-08-18 实测确认，两个方向都验过）。改错这里的后果是「某服务静默不更新」，比排队难查——
调整后务必推一个只动该服务目录的提交实测一次。

验证（**必做**）：轮询 `/api/v1/health`，确认返回的 `git_commit_sha` 已切到新提交，再验证业务行为。

⚠️ **先判断这次提交是否应该触发后端重建**：若只改了文档/测试/`deploy/`，hash **本来就不会变**，
那不是部署失败。确实该重建时，**正常窗口是十几分钟到一小时**——多个服务会并发排队，
2026-08-18 实测 7 个服务排队约 45 分钟、5 个服务约 12 分钟。别在 10 分钟没动就当失败。
真失败通常是 preDeploy 迁移挂了；本次提交若没有新迁移，先去面板看构建日志而不是查迁移。
