...
const SELLER_TARGET_FIELD_LABELS: Record<string, string> = {
  ...
  // 三个退役列（0908 行业字典下线）：更新记录里的历史条目还会引用它们。
  industry_l1: '行业大类（已退役）',
  industry_l2: '细分赛道（已退役）',
  industry_pairs_json: '所属行业（已退役）',
...
const BUYER_INTENT_FIELD_LABELS: Record<string, string> = {
  ...
  intent_business_tags_json: '需求业务标签',
  intent_business_summary: '需求业务说明',
  excluded_business_text: '排除业务方向',
  acceptable_regions_json: '可接受地区',
  excluded_regions_json: '排除地区',
  intent_summary: '意向摘要（已退役）',
  parsed_requirement_json: '结构化需求',
  industry_primary: '一级行业（已退役）',
  industry_secondary: '二级行业（已退役）',
  industries_json: '关注行业（已退役）',
  excluded_industries_json: '排除行业（已退役）',
  industry_focus_tags_json: '细分赛道（已退役）',
  region_scope_summary: '区域范围',
  region_constraints_json: '区域约束（已退役）',
  min_revenue_yuan: '最低营收',
  min_net_profit_yuan: '最低净利润',
  min_total_profit_yuan: '最低利润总额',
  max_pe: '最高 PE',
  max_ps: '最高 PS（已退役）',
  min_net_margin: '最低净利率（已退役）',
  min_gross_margin: '最低毛利率（已退役）',
  ...
  preferred_listed_status: '偏好上市状态（已退役）',
  listing_board_requirement_summary: '板块要求',
  industry_l2_json: '关注细分赛道（已退役）',
...
Object.assign(VALUE_LABELS, {
  ...
  // 现行列。闭集只有三个取值（any / unknown 是已退役的 preferred_listed_status
  // 那一列才有的档），所以不是上面那张表的别名，取值范围确实不同。
