...
#### 补注（2026-08-18，施工中发现）：这一条的 `0.6` 单位写错了

**负债率两侧存的都是百分数**（标的侧实测 `current_debt_ratio` 9.55~75，买家侧同口径，
见《指标体系与推荐对齐方案0817.md》§3.3），60% 要写 `60`。上面那句
「没有一家 ≤ 0.6」是**单位造成的**，不是数据状态——按 60 算全库有 14 家达标。

写 `0.6` 的后果不是「筛得更严」，是**一家也筛不到且不报错**，而 agent 看到的
淘汰拆分会说「确实不达标 16 家」，于是得出「这批标的负债率普遍偏高」这个假结论。
schema 的 description 已改成显式声明单位（`百分数，60% 写 60`），并且新增了一条
构建期断言：**任何 ratio 字段没声明单位就直接报错**，因为百分数与分数在 SQL 里
都不报错，只会让条件恒空或恒真。

用例本身**照原样保留**（`tests/test_screening_sql.py::test_the_relaxation_signal_...`），
它验的是 marginal 语义与缺失/不达标拆分，与单位无关；旁边另加了一条正确单位的
同条件用例（`test_the_debt_ratio_condition_is_read_as_a_percentage`），期望是
命中 **1** 家、`max_debt_ratio` 拆分为 `{总计: 5, 字段为空: 5, 确实不达标: 0}`。
两条都留着：一条钉语义，一条钉单位。
