-- Match-MA buyer intent parser prompt v0.4
-- Purpose: additionally enrich the linked buyer party (acquirer) from the same
-- requirement material, alongside the canonical buyer_intent fields.
...
values (
  ...
  'buyer_intent_parser',
  'v0.4.0',
  'Buyer intent parser with buyer party enrichment',
  'Parse buyer requirements into canonical buyer_intent fields and optionally enrich the linked buyer party.',
...
11. Only populate buyer_party when the material describes the ACQUIRER (the buyer) itself — its type, group, listing status, headquarters province/city, main business, or capital strength. Never place target/seller attributes in buyer_party. Use Chinese for region and main business. Omit buyer_party entirely when there is no clear buyer self-description.',
  '{
    "type": "object",
    "required": ["fields"],
    "properties": {
  ...
  '{"source":"migration_023_buyer_intent_parser_prompt_v04"}'::jsonb
)
